...
namespace margelo::nitro::customcrop {
  ...
  /**
   * Represents the Java/Kotlin callback `(image: String) -> Unit`.
  ...
  public:
     ...
     */
    void invoke_cxx(jni::alias_ref<jni::JString> image) {
      _func(image->toStdString());
    }

  public:
    [[nodiscard]]
    inline const std::function<void(const std::string& /* image */)>& getFunction() const {
      return _func;
    }
  ...
  private:
    explicit JFunc_void_std__string_cxx(const std::function<void(const std::string& /* image */)>& func): _func(func) { }
